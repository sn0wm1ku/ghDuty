...
```bash
${CLAUDE_PLUGIN_ROOT}/scripts/discover.sh
```

It runs the three durable queries, dedupes by `repo#number` (unioning the src
tags), migrates any legacy single-file ledger, and applies the skip-ledger
fast-skip — all in one place (`scripts/discover.sh`, the single source of truth
for the discovery logic, not re-inlined here). It writes the survivor work-set to
`/tmp/ghd_items.json` and prints `survivors: N · fast-skipped: M`. Pass the file's
contents as `args.items` to the workflow (Step 3).
